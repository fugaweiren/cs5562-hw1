...
class ResnetPGDAttacker:
    ...
    def pgd_attack(self, image, label, eps=None, alpha=None, steps=None):
        ...
        adv_images += torch.empty_like(adv_images).uniform_(-self.eps, self.eps).to(self.device) # TODO 1
        ...
        for _ in range(steps):
            ...
            # Gradient update
            # Computing the adversarial image sample with the steepest ascent 
            adv_images = adv_images + alpha * torch.sign(grad)  # TODO 2
            
            # Projection step
            # Clamping the adversarial image within the eps ball
            clamped_distances = torch.clamp(adv_images-images, min=-self.eps, max=self.eps).detach() # TODO 3
            adv_images = images+clamped_distances
